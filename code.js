...
function createDesignElement(instruction, parent) {
    return __awaiter(this, void 0, void 0, function* () {
        let node;
        switch (instruction.type.toLowerCase()) {
            ...
            case 'text':
                ...
                if (instruction.properties.fontName) {
                    yield figma.loadFontAsync(instruction.properties.fontName);
                }
                else {
        ...
        if (instruction.children && node.type === 'FRAME') {
            for (const child of instruction.children) {
                const childNode = yield createDesignElement(child, node);
                if (childNode) {
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    if (msg.type === 'set-api-key') {
        ...
        try {
            const instructions = yield callOpenAI(msg.prompt);
            ...
            for (const instruction of instructions) {
                const element = yield createDesignElement(instruction);
                if (element) {
